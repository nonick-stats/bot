...
export const holder = new PlaceHolder<Hive.AllGameStats>()
  .register(
    'victoryRate',
    ({ played, m_solo_played, victories, m_solo_victories }) =>
      `${Math.round(
        rate(victories || m_solo_victories, played || m_solo_played) * 100,
      )} %`,
  )
  .register('killRate', ({ deaths, kills, murders }) =>
    rate(kills || murders, deaths).toFixed(2),
  )
  .register(
    'hitsRate',
    ({ projectiles_fired, kills }) =>
      `${Math.round(rate(kills, projectiles_fired) * 100)} %`,
  )
  .register(
    'bridge_played',
    ({ m_solo_played, played }) => m_solo_played || played || '0',
  )
  .register(
    'bridge_victories',
    ({ m_solo_victories, victories }) => m_solo_victories || victories || '0',
  )
  .register(
    'bridge_goals',
    ({ m_solo_goals, goals }) => m_solo_goals || goals || '0',
  )
  .register(
    'bridge_kills',
    ({ m_solo_kills, kills }) => m_solo_kills || kills || '0',
  );
